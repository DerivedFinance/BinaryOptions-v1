    ...
    <div className="main-table-wrapper">
            ...
            {activity.map(({ date, type, position, amount, tx }, idx) => {
              return (
                <tr key={idx}>
                  ...
                  <td>{position}</td>
                  <td>{amount}</td>
                  <td onClick={() => openTX("https://testnet.bscscan.com/tx/" + tx)}>
                    <p className="tx-link">link&nbsp;</p>
                    <img src={Link} alt="" className="image-link tx-link" />
                  </td>
                </tr>
              );
            })}
      ...
      {len > 10 && (
        <div className="pagination">
          <p onClick={() => changeOffset("dec")} className={pageOffset === 0 ? "disable" : ""}>
            <ChevronLeftIcon />
          </p>
          <p className="number">{parseInt(pageOffset / 10 + 1)}</p>
          <p onClick={() => changeOffset("inc")} className={pageOffset + 10 > len ? "disable" : ""}>
            <ChevronRightIcon />
          </p>
        </div>
      )}
